namespace SkinApplicationService {
    ...
    bool ApplySkinFromMediaItem(MediaItem@ mediaItem, const string &in layer = "bg") {
        if (mediaItem is null || mediaItem.key.Length == 0) {
            Logging::Error("ApplySkinFromMediaItem: MediaItem is null or key is empty", true);
            return false;
        }
        ...
        if (State::selectedBlock !is null) {
            ...
            Logging::Warn("ApplySkinFromMediaItem: No block or item selected", true);
            return false;
        }
    }
    ...
    bool ApplyBlockSkin(CGameCtnBlock@ block, const string &in skinUrl, const string &in layer) {
        ...
        if (pluginMap is null) return false;
        wstring skinUrlW = wstring(skinUrl);
        wstring currBg = "";
        wstring currFg = "";
        try { currBg = pluginMap.GetBlockSkinBg(block); } catch {}
        try { currFg = pluginMap.GetBlockSkinFg(block); } catch {}
        ...
        try {
            ...
            Logging::Error("ApplyBlockSkin: Exception: " + getExceptionInfo(), true);
            return false;
        }
    }
    ...
    bool ApplyItemSkin(CGameCtnEditorScriptAnchoredObject@ item, const string &in skinUrl, const string &in layer) {
        ...
        wstring currBg = "";
        wstring currFg = "";
        try { currBg = pluginMap.GetItemSkinBg(item); } catch {}
        try { currFg = pluginMap.GetItemSkinFg(item); } catch {}
        wstring newBg = (layer == "bg" || layer == "all") ? skinUrlW : currBg;
        wstring newFg = (layer == "fg") ? skinUrlW : (layer == "all" ? wstring("") : currFg);
        try {
            ...
            Logging::Error("ApplyItemSkin: Exception: " + getExceptionInfo(), true);
            return false;
        }
    }
